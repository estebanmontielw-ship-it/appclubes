    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 via-white to-red-50 px-4">
      <Card className="w-full max-w-md shadow-lg border-0 shadow-gray-200/50">
        <CardHeader className="text-center pb-2">
          ...
          <CardTitle className="text-2xl font-bold text-gray-900">Bienvenido a CPB</CardTitle>
          <CardDescription className="text-gray-500">Confederación Paraguaya de Básquetbol</CardDescription>
        </CardHeader>
        <form onSubmit={handleSubmit(onSubmit)}>
          <CardContent className="space-y-4 pt-4">
            <div className="space-y-2">
              <Label htmlFor="email" className="text-gray-700">Email</Label>
              <Input id="email" type="email" placeholder="tu@email.com" className="h-11" {...register("email")} />
              {errors.email && <p className="text-sm text-destructive">{errors.email.message}</p>}
            </div>
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <Label htmlFor="password" className="text-gray-700">Contraseña</Label>
                <Link href="/oficiales/recuperar" className="text-xs text-primary hover:underline font-medium">
          ...
          <div className="relative bg-white rounded-2xl p-6 max-w-sm w-full shadow-2xl z-10" onClick={e => e.stopPropagation()}>
            <div className="text-center mb-4">
              ...
              <p className="text-sm text-muted-foreground mt-2">
                El email o contraseña no coinciden. ¿Puede ser que todavía no te hayas registrado?
              </p>
            </div>
            <div className="space-y-2">
